...
def _serialize_result(item: ScanResult) -> dict:
    return {
        ...
        "technical": {
            "ema20": item.snapshot.ema20 if item.snapshot else None,
            "ema50": item.snapshot.ema50 if item.snapshot else None,
            "rsi14": item.snapshot.rsi14 if item.snapshot else None,
            "atr14": item.snapshot.atr14 if item.snapshot else None,
            "relative_volume20": item.snapshot.relative_volume20 if item.snapshot else None,
            "resistance20": item.snapshot.resistance20 if item.snapshot else None,
            "support20": item.snapshot.support20 if item.snapshot else None,
        },
        "evidence": {
...
def _serialize_report(report: MarketScanReport) -> dict:
    return {
        ...
        "analysis_count": report.universe_size - len(report.errors),
        "coverage_percent": round(100.0 * (report.universe_size - len(report.errors)) / report.universe_size, 1) if report.universe_size else 0.0,
